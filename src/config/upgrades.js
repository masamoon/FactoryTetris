...
export const upgradesConfig = {
  [UPGRADE_TYPES.RESOURCE_BOUNTY]: {
    ...
    tiers: [
      ...
      { level: 2, modifier: 1.6, description: '+60% Starting Resources' },
      { level: 3, modifier: 2.0, description: '+100% Starting Resources' },
    ],
  ...
  [UPGRADE_TYPES.RESOURCE_REGEN]: {
    ...
    tiers: [
      { level: 1, modifier: 1.2, description: '+20% Replenish Rate' },
      { level: 2, modifier: 1.5, description: '+50% Replenish Rate' },
      { level: 3, modifier: 2.0, description: '+100% Replenish Rate' },
    ],
  ...
  [UPGRADE_TYPES.PROCESSOR_EFFICIENCY]: {
    ...
    tiers: [
      ...
      { level: 2, modifier: 1.4, description: '+40% Processor Speed' },
  ...
  [UPGRADE_TYPES.NODE_LONGEVITY]: {
    ...
    tiers: [
      ...
      { level: 2, modifier: 1.4, description: '+40% Node Lifespan' },
      { level: 3, modifier: 2.0, description: '+100% Node Lifespan' },
    ],
  ...
  [UPGRADE_TYPES.INVENTORY_CAPACITY]: {
    ...
    tiers: [
      ...
      { level: 2, modifier: 1.5, description: '+50% Inventory Capacity' },
      { level: 3, modifier: 2.0, description: '+100% Inventory Capacity' },
    ],
  ...
  [UPGRADE_TYPES.EXTRACTION_SPEED]: {
    ...
    tiers: [
      ...
      { level: 2, modifier: 0.5, description: '50% Faster Extraction' },
      ...
      BaseMachine.prototype.completeProcessing = function () {
        ...
        if (this.id && this.id.includes('processor')) {
          ...
          if (Math.random() < 0.2 && this.outputTypes) {
            console.log(
              `[QUANTUM SPLITTER] Activated on ${this.id} at (${this.gridX}, ${this.gridY})`
            );
            console.log(`[QUANTUM SPLITTER] Before processing: ${JSON.stringify(beforeState)}`);
            console.log(
              `[QUANTUM SPLITTER] After normal processing: ${JSON.stringify(afterNormalProcessing)}`
            );
            ...
            console.log(
              `[QUANTUM SPLITTER] After quantum effect: ${JSON.stringify(this.outputInventory)}`
            );
            ...
            if (this.scene) {
              ...
              const floatingText = this.scene.add
                .text(this.container.x, this.container.y - 20, 'QUANTUM!', {
                  fontFamily: 'Arial',
                  fontSize: '18px',
                  fontWeight: 'bold',
                  color: '#bb44ff',
    ...
    effect: (_game) => {
      ...
      BaseMachine.prototype.canProcess = function () {
        ...
        if (wouldUseOverflow && result) {
          console.log(
            `[OVERFLOW BUFFER] Activated on ${this.id} at (${this.gridX}, ${this.gridY})`
          );
          console.log(
            `[OVERFLOW BUFFER] Current inventory: ${currentLevel}/${originalCapacity} (normal cap)`
          );
          console.log(
            `[OVERFLOW BUFFER] Using extended capacity: ${currentLevel}/${originalCapacity + 2} (with overflow)`
          );
          ...
          if (this.scene && (!this._lastOverflowState || !this.overflowIndicator)) {
            ...
            const floatingText = this.scene.add
              .text(this.container.x, this.container.y - 20, 'OVERFLOW!', {
                fontFamily: 'Arial',
                fontSize: '18px',
                fontWeight: 'bold',
                color: '#ff9900',
        ...
        if (result && this.outputTypes) {
          ...
          if ((this.outputInventory[outputType] || 0) >= originalCapacity) {
            ...
            if (this.container && !this.overflowIndicator) {
              this.overflowIndicator = this.scene.add
                .sprite(
                  0,
                  -25,
                  'particle' // Using particle as placeholder
                )
                .setScale(0.5)
                .setTint(0xff9900);
