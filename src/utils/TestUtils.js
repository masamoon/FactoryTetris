...
export default class TestUtils {
  ...
  runAutomatedTests() {
    ...
    try {
      ...
      this.logTestResult(
        'new-machine-classes',
        newMachineClassesResult,
        newMachineClassesResult
          ? 'New machine classes working correctly'
          : 'New machine classes test failed'
      );
  ...
  testConveyorPlacement() {
    ...
    const indicatorRotation = machine.directionIndicator
      ? machine.directionIndicator.rotation
      : null;
  ...
  testRotation() {
    ...
    if (!machineType) {
      this.logTestResult(
        'rotation-test',
        false,
        'Conveyor machine type not found for rotation test'
      );
      return;
    }
  ...
  displayResultsInGame(passCount, failCount) {
    ...
    const title = this.scene.add
      .text(width / 2, height * 0.2, 'AUTOMATED TEST RESULTS', {
        fontFamily: 'Arial',
        fontSize: 24,
        color: '#ffffff',
        fontStyle: 'bold',
    ...
    const summary = this.scene.add
      .text(width / 2, height * 0.3, `Tests completed: ${passCount} passed, ${failCount} failed`, {
        fontFamily: 'Arial',
        fontSize: 18,
    ...
    this.testResults.forEach((result, index) => {
      ...
      const text = this.scene.add
        .text(0, yOffset, `[${statusText}] ${result.name}`, {
          fontFamily: 'Arial',
          fontSize: 16,
          color: statusColor,
        })
  ...
  autoFixDirectionIndicators() {
    ...
    const rotationTests = this.testResults.filter(
      (result) => result.name.startsWith('rotation-') && !result.passed
    );
  ...
  applyDirectionIndicatorFix(directionMappings) {
    ...
    this.scene.updateDirectionIndicator = (machine, direction) => {
      ...
      console.log(
        `[GameScene] Fixing direction indicator for machine type: ${machine.type.name}, direction: ${direction}`
      );
  ...
  fixMachineDirectionIndicator() {
    ...
    this.scene.factoryGrid.placeMachine = (machineType, gridX, gridY, rotation) => {
      ...
      const machine = originalCreateMethod.call(
        this.scene.factoryGrid,
        machineType,
        gridX,
        gridY,
        rotation
