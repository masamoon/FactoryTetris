...
export function assignLevelsToShape(shape, scene, options = {}) {
  ...
  const previewOutputLevel = getDraftOutputLevel(config, producibleLevels);

  const result = {
    inputLevels: isArithmeticConfig(config) ? [] : [...config.inputs],
    outputLevel: isArithmeticConfig(config) ? null : config.output,
    previewOutputLevel,
