...
export default class ProcessorAMachine extends BaseMachine {
    ...
    initMachineProperties() {
        ...
        const ioPositions = ProcessorAMachine.getIOPositionsForDirection('processor-a', this.defaultDirection);
    ...
    createVisuals() {
        ...
        if (!this.container) {
            console.error(`[${this.id}] Container not created by super.createVisuals(). Aborting specific visuals.`);
            return;
        }
        ...
        const machineLabel = this.scene.add.text(visualCenterX, visualCenterY, "A", {
            fontFamily: 'Arial',
            fontSize: 14,
            color: '#ffffff'
        }).setOrigin(0.5);
