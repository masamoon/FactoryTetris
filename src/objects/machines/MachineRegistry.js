...
export default class MachineRegistry {
  ...
  createMachine(id, scene, config) {
    ...
    console.log(
      `[MachineRegistry] Found constructor for ${id}:`,
      MachineConstructor ? 'Yes' : 'No'
    ); // Log constructor found
    if (!MachineConstructor) {
      // Extra safety check
      console.error(
        `[MachineRegistry] Constructor for ${id} was null/undefined despite passing hasMachineType check!`
      );
      return null;
    }
    ...
    try {
      ...
      console.log(
        `[MachineRegistry] --> Calling constructor with scene: ${scene ? scene.scene.key : 'INVALID'}, config keys: ${config ? Object.keys(config).join(', ') : 'N/A'}`
      );
      ...
      console.error(
        `[MachineRegistry] Error during new ${MachineConstructor.name}(...) for ID ${id}:`,
        error
      ); // Log error during new()
  ...
  createMachinePreview(id, scene, x, y, direction = 'right') {
    ...
    if (MachineConstructor.getPreviewSprite) {
      console.log(
        `[MachineRegistry] Using STATIC getPreviewSprite for ${id} with direction ${direction}`
      );
      ...
      console.log(
        `[MachineRegistry] Static getPreviewSprite NOT FOUND for ${id}. Falling back to instance method.`
      );
