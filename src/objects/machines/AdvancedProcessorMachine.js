...
export default class AdvancedProcessorMachine extends BaseMachine {
  ...
  logInputStatus() {
    ...
    if (adjacentMachines.length > 0) {
      console.log(
        `[AdvancedProcessor] Adjacent machines:`,
        adjacentMachines
          .map(
            (m) =>
              `${m.id} at (${m.gridX},${m.gridY}) with outputs: ${JSON.stringify(m.outputInventory)}`
          )
          .join(', ')
      );
    } else {
    ...
    for (const cell of occupiedCells) {
      ...
      for (const dir of directions) {
        ...
        try {
          ...
          if (
            neighborCell &&
            neighborCell.type === 'machine' &&
            neighborCell.object &&
            neighborCell.object !== this &&
            !adjacentMachines.includes(neighborCell.object)
          ) {
  ...
  initMachineProperties() {
    ...
    const ioPositions = AdvancedProcessorMachine.getIOPositionsForDirection(
      'advanced-processor',
